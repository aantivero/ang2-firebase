/**
 * Created by alejandro on 12/3/2017.
 */
...
let version = 1;
let cacheName = 'debin1-pwa-client-v' + version;
let dataCacheName = 'debin1-pwa-client-data-v' + version;
  ...
  e.waitUntil(
    caches.keys().then((keyList) => {
      return Promise.all(keyList.map((key) => {
        ...
        if (key !== cacheName) {
          log('Service Worker: Remove old cache', key);
